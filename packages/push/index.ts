...
import { stringify } from '@mikotojs/utils'
import process from 'node:process'
import { conciseNickname } from './utils'
...
function initEnv() {
  ...
  function upperCaseToHump(str: string) {
    return str
      .toLowerCase()
      .replace(/_(\w)/g, (_match, t: string) => t.toUpperCase())
  }
...
async function sendMail(title: string, text: string) {
  ...
  if (!user || !user.pass || !user.from || !user.host) {
    return
  }
...
async function customApi(title: string, text: string) {
  try {
    ...
    if (!apiTemplate || !apiTemplate.url) {
      return
    }
...
function iGotNotify(text, desp, params = {}) {
  return new Promise((resolve) => {
    ...
    if (IGOT_PUSH_KEY) {
      ...
      const IGOT_PUSH_KEY_REGX = /^[a-zA-Z0-9]{24}$/
